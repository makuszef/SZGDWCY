...
import { Link, useNavigate } from 'react-router-dom';
import axios from 'axios';
import { useAuth } from './AuthContext.jsx';  // To jest poprawny import
...
const LoginPage = () => {
    ...
    const [openSnackbar, setOpenSnackbar] = useState(false);
    const [successMessage, setSuccessMessage] = useState('');
    const { login } = useAuth(); // Korzystamy z funkcji login z kontekstu
    const navigate = useNavigate();

    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('https://localhost:7191/login', { email, password });
            ...
            setTimeout(() => {
                navigate('/gospodarstwa'); // Zmieniamy na odpowiednią trasę
            }, 2000); // Przekierowanie po 2 sekundach
